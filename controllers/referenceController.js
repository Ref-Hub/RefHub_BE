...
export const updateReference = async (req, res) => {
  try {
    ...
    let keepFilePaths = [];
    try {
      keepFilePaths = existingFiles
        ? JSON.parse(existingFiles)
        : reference.files.map((f) => f.path);
    } catch {
      return res
        .status(400)
        .json({ error: "기존 파일 정보 형식이 잘못되었습니다." });
    }
    ...
    const filesToDelete = reference.files.filter(
      (file) =>
        !filesToKeep.find((f) => f._id.toString() === file._id.toString())
    );
    ...
    for (const file of filesToDelete) {
      ...
      if (file.type !== "link" && file.path) {
        const paths = file.path.includes(",")
          ? file.path.split(",").map((p) => p.trim())
          : [file.path];
    ...
    try {
      const keepKeywordIds = existingKeywords
    ...
    if (keywords) {
      ...
      for (const word of keywordArray) {
        ...
        if (
          !updatedKeywordIds.some((id) => id.toString() === kw._id.toString())
        ) {
    ...
    if (req.files?.files) {
      ...
      for (const file of pdfs) {
        ...
        const preview = await convertPdfToImage({
          ...file,
          originalname: encodedName,
        });
        newFiles.push({
    ...
    // 기타 파일 처리
    if (req.files?.otherFiles) {
      const others = Array.isArray(req.files.otherFiles)
...
export const getReference = async (req, res) => {
  ...
  try {
    ...
    switch (sortBy) {
      case "latest":
        sort = { updatedAt: -1 }; // createdAt → updatedAt
        break;
      case "oldest":
        sort = { updatedAt: 1 }; // createdAt → updatedAt
        break;
